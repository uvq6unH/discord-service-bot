    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2 }}
      ...
      <div style={{ marginBottom: 'var(--s8)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--s3)', marginBottom: 'var(--s2)' }}>
          ...
          <h1 style={{ fontSize: 24, fontWeight: 700, color: 'var(--text-1)', margin: 0, letterSpacing: '-0.02em' }}>Music Services</h1>
        ...
        <p style={{ fontSize: 13, color: 'var(--text-3)', margin: 0 }}>Lavalink · Nodes · Queues · Audio Settings · Playback Config</p>
      </div>

      <SectionCard title="Music System" icon={<Music size={14} strokeWidth={1.75} />}
        toggle={{ enabled: config.musicEnabled ?? false, onChange: v => updateConfig({ musicEnabled: v }) }}>
        <div className="form-group">
          <label className="form-label">Prefix nhạc</label>
          <input className="form-input" value={config.musicPrefix ?? 'hb'}
            onChange={e => updateConfig({ musicPrefix: e.target.value })}
            placeholder="hb" style={{ maxWidth: 160 }} />
        </div>
      </SectionCard>
      ...
      <SectionCard title="Playback Config" icon={<Settings size={14} strokeWidth={1.75} />}>
        <div className="form-group">
          ...
          <input type="range" min={0} max={100} value={music.defaultVolume ?? 70}
