...
function normalizeEsportsLeagues(leagues) {
  ...
  const valid = new Set(['lck', 'lcp', 'lpl', 'lec', 'lcs', 'worlds', 'msi', 'first_stand', 'ewc']);
...
export class ConfigStore {
  ...
  async getGuildConfig(guildId) {
    ...
    const base = {
      ...
      esportsMatchResultAlert: pickBoolean(stored, 'esportsMatchResultAlert', defaultConfig.esportsMatchResultAlert),
  ...
  async updateGuildConfig(guildId, rawPatch) {
    ...
    return this._withLock(guildId, async () => {
      ...
      const next = {
        ...
        autoRoleId: normalizeSnowflakeId(patch.autoRoleId ?? current.autoRoleId),
        ...
        ticketPanelTitle: String(patch.ticketPanelTitle ?? current.ticketPanelTitle ?? '').trim().slice(0, 100) || defaultConfig.ticketPanelTitle,
        ticketPanelMessage: String(patch.ticketPanelMessage ?? current.ticketPanelMessage ?? '').trim().slice(0, 1000) || defaultConfig.ticketPanelMessage,
        levelsEnabled: pickBoolean(patch, 'levelsEnabled', current),
        xpPerMessage: Math.max(1, Math.min(100, Number.parseInt(patch.xpPerMessage ?? current.xpPerMessage, 10) || defaultConfig.xpPerMessage)),
        levelUpMessage: String(patch.levelUpMessage ?? current.levelUpMessage ?? '').trim().slice(0, 500) || defaultConfig.levelUpMessage,
        levelUpAnnouncementEnabled: pickFlag(patch, 'levelUpAnnouncementEnabled', current, { defaultTrue: true }),
        levelUpAnnouncementChannelId: normalizeSnowflakeId(patch.levelUpAnnouncementChannelId ?? current.levelUpAnnouncementChannelId),
        xpBase: Math.max(1, Math.min(100000, Number.parseInt(patch.xpBase ?? current.xpBase, 10) || defaultConfig.xpBase)),
        ...
        currencySilverName: String(patch.currencySilverName ?? current.currencySilverName ?? '').trim().slice(0, 40) || defaultConfig.currencySilverName,
        currencySilverIcon: String(patch.currencySilverIcon ?? current.currencySilverIcon ?? '').trim().slice(0, 8) || defaultConfig.currencySilverIcon,
        currencyGoldName: String(patch.currencyGoldName ?? current.currencyGoldName ?? '').trim().slice(0, 40) || defaultConfig.currencyGoldName,
        currencyGoldIcon: String(patch.currencyGoldIcon ?? current.currencyGoldIcon ?? '').trim().slice(0, 8) || defaultConfig.currencyGoldIcon,
        currencyDiamondName: String(patch.currencyDiamondName ?? current.currencyDiamondName ?? '').trim().slice(0, 40) || defaultConfig.currencyDiamondName,
        currencyDiamondIcon: String(patch.currencyDiamondIcon ?? current.currencyDiamondIcon ?? '').trim().slice(0, 8) || defaultConfig.currencyDiamondIcon,
        dailyEnabled: pickFlag(patch, 'dailyEnabled', current),
        dailyCooldownHours: Math.max(1, Math.min(168, Number.parseInt(patch.dailyCooldownHours ?? current.dailyCooldownHours, 10) || defaultConfig.dailyCooldownHours)),
        ...
        dailySilverAmount: Math.max(0, Math.min(1000000, Number.parseInt(patch.dailySilverAmount ?? current.dailySilverAmount, 10) || 0)),
        dailyGoldAmount: Math.max(0, Math.min(1000000, Number.parseInt(patch.dailyGoldAmount ?? current.dailyGoldAmount, 10) || 0)),
        dailyDiamondAmount: Math.max(0, Math.min(1000000, Number.parseInt(patch.dailyDiamondAmount ?? current.dailyDiamondAmount, 10) || 0)),
        blackjackEnabled: pickFlag(patch, 'blackjackEnabled', current),
        blackjackMinBet: Math.max(1, Math.min(1000000, Number.parseInt(patch.blackjackMinBet ?? current.blackjackMinBet, 10) || defaultConfig.blackjackMinBet)),
        blackjackMaxBet: Math.max(1, Math.min(1000000, Number.parseInt(patch.blackjackMaxBet ?? current.blackjackMaxBet, 10) || defaultConfig.blackjackMaxBet)),
        pokerEnabled: pickFlag(patch, 'pokerEnabled', current),
        pokerMinBet: Math.max(1, Math.min(1000000, Number.parseInt(patch.pokerMinBet ?? current.pokerMinBet, 10) || defaultConfig.pokerMinBet)),
        pokerMaxBet: Math.max(1, Math.min(1000000, Number.parseInt(patch.pokerMaxBet ?? current.pokerMaxBet, 10) || defaultConfig.pokerMaxBet)),
        coinflipEnabled: pickFlag(patch, 'coinflipEnabled', current),
        coinflipMinBet: Math.max(1, Math.min(1000000, Number.parseInt(patch.coinflipMinBet ?? current.coinflipMinBet, 10) || defaultConfig.coinflipMinBet)),
        coinflipMaxBet: Math.max(1, Math.min(1000000, Number.parseInt(patch.coinflipMaxBet ?? current.coinflipMaxBet, 10) || defaultConfig.coinflipMaxBet)),
        diceEnabled: pickFlag(patch, 'diceEnabled', current),
        diceMinBet: Math.max(1, Math.min(1000000, Number.parseInt(patch.diceMinBet ?? current.diceMinBet, 10) || defaultConfig.diceMinBet)),
        diceMaxBet: Math.max(1, Math.min(1000000, Number.parseInt(patch.diceMaxBet ?? current.diceMaxBet, 10) || defaultConfig.diceMaxBet)),
        slotsEnabled: pickFlag(patch, 'slotsEnabled', current),
        slotsMinBet: Math.max(1, Math.min(1000000, Number.parseInt(patch.slotsMinBet ?? current.slotsMinBet, 10) || defaultConfig.slotsMinBet)),
        slotsMaxBet: Math.max(1, Math.min(1000000, Number.parseInt(patch.slotsMaxBet ?? current.slotsMaxBet, 10) || defaultConfig.slotsMaxBet)),
        announcementsEnabled: pickBoolean(patch, 'announcementsEnabled', current),
        announcementChannelId: normalizeSnowflakeId(patch.announcementChannelId ?? current.announcementChannelId),
        announcementMention: String(patch.announcementMention ?? current.announcementMention ?? '').trim().slice(0, 100),
        welcomeEnabled: pickBoolean(patch, 'welcomeEnabled', current),
        welcomeChannelId: normalizeSnowflakeId(patch.welcomeChannelId ?? current.welcomeChannelId),
        welcomeMessage: String(patch.welcomeMessage ?? current.welcomeMessage ?? '').trim().slice(0, 500) || defaultConfig.welcomeMessage,
        logChannelId: normalizeSnowflakeId(patch.logChannelId ?? current.logChannelId),
        ...
        tempVcMasterChannelId: normalizeSnowflakeId(patch.tempVcMasterChannelId ?? current.tempVcMasterChannelId),
        tempVcCategoryId: normalizeSnowflakeId(patch.tempVcCategoryId ?? current.tempVcCategoryId),
        tempVcControlChannelId: normalizeSnowflakeId(patch.tempVcControlChannelId ?? current.tempVcControlChannelId),
        esportsNotifyEnabled: pickBoolean(patch, 'esportsNotifyEnabled', current),
        esportsChannelId: normalizeSnowflakeId(patch.esportsChannelId ?? current.esportsChannelId),
        ...
        esportsMatchResultAlert: pickBoolean(patch, 'esportsMatchResultAlert', current),
